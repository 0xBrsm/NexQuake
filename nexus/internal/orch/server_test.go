...
func attachInstanceForTest(t *testing.T, m *ServerManager, seedID int, instance *instance) int {
	t.Helper()
	...
	if seedState := s.instanceStates[seedID]; seedState == nil {
		s.instanceStates[seedID] = newInstanceState(instanceLifecycleActive)
	} else {
		transitionInstanceLifecycle(seedState, instanceLifecycleActive)
		seedState.ZeroPollStreak = 0
	}
	if instanceStates := s.instanceStates[instance.id]; instanceStates == nil {
		s.instanceStates[instance.id] = newInstanceState(instanceLifecycleActive)
	} else {
		transitionInstanceLifecycle(instanceStates, instanceLifecycleActive)
		instanceStates.ZeroPollStreak = 0
	}
	m.refreshServerSnapshotLocked(s)
	return s.CandidatePort
}
...
func TestRegisterServerSeed_EnablesAutoscalingWhenServerSizeAboveOne(t *testing.T) {
	...
	if s.CandidatePort != 26000 {
		t.Fatalf("candidate port = %d, want 26000", s.CandidatePort)
	}
}
...
func TestServerRouting_PicksLeastLoadedInstance(t *testing.T) {
	...
	port, ok := m.pickServerInstanceLocked(s)
	m.mu.Unlock()
...
func TestServerRouting_UsesDrainingInstanceWhenOnlyFreeSlotsRemain(t *testing.T) {
	...
	if state == nil {
		state = newInstanceState(instanceLifecycleActive)
		s.instanceStates[replica.id] = state
	}
	transitionInstanceLifecycle(state, instanceLifecycleDraining)
	replicaDraining := state != nil && state.Lifecycle == instanceLifecycleDraining
	routed, ok := m.pickServerInstanceLocked(s)
	m.mu.Unlock()
...
func TestServerRouting_AllDrainingInstancesRemainRoutable(t *testing.T) {
	...
	for _, serverID := range s.InstanceIDs {
		...
		if state == nil {
			state = newInstanceState(instanceLifecycleDraining)
...
func TestServerRouting_SkipsWarmingInstanceForNewSessions(t *testing.T) {
	...
	transitionInstanceLifecycle(state, instanceLifecycleWarming)
	routed, ok := m.pickServerInstanceLocked(s)
	m.mu.Unlock()
