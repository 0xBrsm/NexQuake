...
func (m *ServerManager) serverRoutableCandidatesLocked(s *server, allowDraining bool) []serverInstanceCandidate {
	...
	for _, serverID := range s.InstanceIDs {
		...
		if s.Autoscales {
			...
			if !instanceAllowsRouting(state, allowDraining) {
				continue
			}
		}
...
func (m *ServerManager) serverRoutableCandidateCountLocked(s *server, allowDraining bool) int {
	if s == nil {
		return 0
	}
...
func (m *ServerManager) pickServerInstanceLocked(s *server) (int, bool) {
...
func pruneServerDemandLocked(s *server, now time.Time) {
	...
	s.joinDemandAt = s.joinDemandAt[:n]
}
...
func (m *ServerManager) decideServerActionsLocked(s *server, now time.Time) (scaleUpServerID int, despawnServerID int) {
	...
	activeRoutableCount := m.serverRoutableCandidateCountLocked(s, false)
	...
	for _, serverID := range s.InstanceIDs {
		...
		setLifecycle := func(next instanceLifecycle) {
			...
			transitionInstanceLifecycle(state, next)
...
func (m *ServerManager) launchServerReplica(serverID int) {
	...
	if err != nil {
		m.warnf("Server %d scale-up failed: %v", serverID, err)
		return
	}
	if rec == nil {
		return
	...
	m.mu.RUnlock()
	if line >= 0 {
		m.infof("Server %d line %d launched replica", serverID, line+1)
		return
	}
	m.infof("Server %d launched replica", serverID)
}
...
func (m *ServerManager) despawnServerInstance(serverID int) {
	...
	if s == nil || rec == nil || !m.instanceRunningLocked(rec) {
		...
		if s := m.serverByInstanceID[serverID]; s != nil {
			m.setServerInstanceLifecycleLocked(s, serverID, instanceLifecycleWarming, false)
		}
		m.mu.Unlock()
		return
	}
	if m.serverRunningCountLocked(s) <= 1 {
		...
		m.mu.Lock()
		if s := m.serverByInstanceID[serverID]; s != nil {
			m.setServerInstanceLifecycleLocked(s, serverID, instanceLifecycleActive, false)
		}
		m.mu.Unlock()
		return
	}
	...
	cancel()
	if err != nil {
		m.warnf("Server %d despawn failed: %v", serverID, err)
	}
