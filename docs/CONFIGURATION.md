...
NexQuake is designed to be drop-in compatible with any protocol 15 (NetQuake) dedicated server binary. The bundled `nqserver` is the stock WinQuake engine with minimal patches, but you can substitute any conformant binary by placing it in `BIN_DIR`.
...
| `CLIENT_DIR` | `/app/client` | Location of WASM client assets (`index.wasm`, `index.html`). |
