...
Native **follows the window live**: `VID_Update()` recomputes the target each frame and, when it drifts from the current render, debounces a `VID_SetMode()` re-mode (orientation flips snap; gradual drags coalesce). The check runs every frame rather than only on canvas-resize, because mobile `innerWidth/innerHeight` can lag the canvas reflow after a rotation. The selected mode index is mirrored to `localStorage` and read in `VID_Init()`, so the engine boots directly into the last mode instead of the compiled default.
