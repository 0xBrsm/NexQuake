...
Get a NexQuake server running and play in your browser.
...
That's it. Nexus bootstraps game data automatically on first boot using the default manifest, which includes the Quake 1.06 shareware and an open-source [LibreQuake](https://github.com/lavenderdotpet/LibreQuake) PAK1. This gives you a registered engine with Episode 1 single-player and full multiplayer mod support.
...
```bash
docker run -p 1337:1337 -e QUICKSTART=full ghcr.io/0xbrsm/nexquake
```
