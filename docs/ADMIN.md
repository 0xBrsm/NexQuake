...
| `rcon login` | Browser/WASM client only; no JSON-RPC equivalent. **Direct exposure** (`EXTERNAL_URL` set, default `AUTH_JWT_HEADER`): runs an Authorization Code + PKCE login — the popup authorizes at the IdP, Nexus exchanges the code at `POST /rcon/session` and sets the httpOnly `nq_session` cookie. **Behind a front:** opens a popup on `GET /rcon` so the access gate (e.g. Cloudflare Access on `/rcon`) runs its IdP login and sets its cookie. Either way the `GET /rcon` landing page relays the outcome to the shell via `localStorage`, which prints the result to the in-game console (and, on touch devices, also shows a toast). |
...
- **GET /rcon**: returns a small auto-closing HTML page. The WASM shell opens it as a top-level popup so a fronting access gate can run its IdP round trip and set its cookie. The page relays the login outcome to the opener through `localStorage` (picked up via a `storage` event), which survives the COOP-same-origin severance of `window.opener` that breaks postMessage. For the edge-gated flow Nexus embeds the authorization result — computed with the same admin check as POST — into the page; for the PKCE flow the page relays the OAuth `code` for the shell to exchange at `POST /rcon/session`. localStorage is per-origin and read only by the same browser, so there's no cross-session signal to abuse.
