...
NexQuake is a WebAssembly port of Quake with browser-native multiplayer. It runs the original id Software engine in the browser and connects players back to dedicated game servers on the host through a lightweight Go relay that tunnels UDP over WebSocket or WebTransport.

```
Browser Client  --WebSocket/WebTransport-->  Nexus  --UDP-->  NetQuake Server
```
...
**Nexus** is a Go orchestration server that serves client files, serves game data, manages servers, and tunnels multiplayer traffic. Each tunnel frame (WebSocket message or WebTransport datagram) carries a small routing header plus a raw NetQuake `protocol 15` datagram. Nexus acts as a transparent relay with multi-server routing; it never parses game packets.
...
client/           WASM client platform layer and tunnel networking (WebSocket/WebTransport)
