...
// Forward declaration (required by modern clang/emcc when passing as a callback).
void main_loop(void);
#ifdef __EMSCRIPTEN__
...
//	signal(SIGFPE, floating_point_exception_handler);
    ...
    Con_Printf("Based on the SDL patch from libsdl.org\n\n");
    Con_Printf("SDL2 & WASM conversion by Gregory Maynard-Hoare\n\n");
